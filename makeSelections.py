...
ifile = "/Users/yfeng/Desktop/TTU/CaloX/Data/run571_250607165307_converted.root"
ifile = "/Users/yfeng/Desktop/TTU/CaloX/Data/run583_250609182256_converted.root"
suffix = "run583"
infile = ROOT.TFile(ifile, "READ")
rdf = ROOT.RDataFrame("EventTree", infile)

tree = infile.Get("EventTree")
for board in range(0, 13):
...
for board in [10]:
    for iCer, iSci in map_Cer_Sci.items():
        hist = rdf.Histo2D((
            ...
            f"FERS_Board{board}_energyHG_{iCer}",
            f"FERS_Board{board}_energyHG_{iSci}"
        )
...
outfile = ROOT.TFile(f"root/fers_all_channels_{suffix}.root", "RECREATE")
for hist in hists1d:
    hist.Write()
for hist in hists2d:
    hist.Write()
...
outfile_DRS = ROOT.TFile(f"root/drs_all_channels_{suffix}.root", "RECREATE")
...
outfile_hodos = ROOT.TFile(
    f"root/hodoscope_all_channels_{suffix}.root", "RECREATE")
...
rdf_filtered.Snapshot(
    "EventTree", f"root/filtered_events_{suffix}.root")
